...
def run():
    ...
    xmin = min(out14[0])
    xmax = max(out14[0])
    ymin = min(min(out14[1]), min(out16[1]))
    ymax = max(max(out14[1]), max(out16[1]))
    ydiff = ymax - ymin
    ymin -= 0.2 * ydiff
    ymax += 0.2 * ydiff
    ...
    goes14, = plt.plot(out14[0], out14[1], color='red',  marker='.', markersize=0, lw=0.8, label='GOES14')
    goes16, = plt.plot(out16[0], out16[1], color='blue', marker='.', markersize=0, lw=0.8, label='GOES16')

    plt.legend(handles=[goes14, goes16])
...
def get_hrc_p_14():
    ...
    data   = mcf.read_data_file('./goes14_diff')
    ktime  = []
    pdata  = [[],[],[],[],[],[],[],[],[],[],[]]
