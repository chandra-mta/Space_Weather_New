...
c modified may 10, 2018 by TI --- input file location changed
...
c----------------
c f77 crmreg_xmm.f 
c   /data/mta4/Space_Weather/CRMFLX/CRMFLX_V23_EX/geopack.f
c   /data/mta4/Space_Weather/CRMFLX/CRMFLX_V23_EX/supple.f  -o crmreg_xmm
c
c       last update: Aug 23, 2018               TI
c
...
c      open(unit=1,name='/proj/rac/ops/ephem/TLE/xmm.gsme_in_Re')
c      open(unit=1,name='/data/mta4/proj/rac/ops/ephem/TLE/xmm.gsme_in_Re')

      open(unit=1,name='./xmm.gsme_in_Re')
      open(unit=2,
     &name='/data/mta4/Space_Weather/XMM/Data/crmreg_xmm.dat')

      do 100 while (ios.eq.0)
         read(1,*,iostat=ios) t,xgse,ygse,zgse,Xgsm,Ygsm,Zgsm,fy,mon,d,
     &h,min,s
