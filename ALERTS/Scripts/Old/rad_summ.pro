...
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
FUNCTION calc_grat_att, startt,stopt,ifot_dir
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

; find attenuation factor between startt and stopt (in julian days)

fline = ifot_dir + 'grating/*.rdb'
files = findfile(fline)
...
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
FUNCTION calc_att, startt,stopt, ifot_dir
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

; find attenuation factor between startt and stopt (in julian days)
iline = ifot_dir + 'sim/*.rdb'
files = findfile(iline)
...
print,"SIM ",i, rdb_dat(i).SIMTRANS_POS
  rdb_time = strsplit(rdb_dat(i).TStart_GMT,":",/extract)
  yr=rdb_time(0)
  dd=rdb_time(1)
  hh=rdb_time(2)
  ...
  if (rdb_start_test lt startt and rdb_stop_test gt startt and rdb_stop_test lt stopt) then begin
    if (rdb_dat(i).SIMTRANS_POS gt 0L) then begin
      att_time=att_time+calc_grat_att(startt,rdb_stop_test,ifot_dir)
      print, "ACIS 1 ", rdb_dat(i).SIMTRANS_POS, att_time
    endif
  endif
  if (rdb_start_test lt startt and rdb_stop_test gt startt and rdb_stop_test gt stopt) then begin
    if (rdb_dat(i).SIMTRANS_POS gt 0L) then begin
      att_time = att_time+calc_grat_att(startt,stopt,ifot_dir)
  ...
  if (found eq 0 and rdb_start_test gt startt and rdb_stop_test lt stopt) then begin
    if (rdb_dat(i).SIMTRANS_POS gt 0L) then begin
      att_time = att_time+calc_grat_att(rdb_start_test,rdb_stop_test,ifot_dir)
      print, "ACIS 3", att_time
    endif
  endif
  if (found eq 0 and rdb_start_test gt startt and rdb_stop_test gt stopt) then begin
    if (rdb_dat(i).SIMTRANS_POS gt 0L) then begin
      att_time = att_time+calc_grat_att(rdb_start_test,stopt,ifot_dir)
...
web_dir      = html_dir + 'Alerts/'
chandra_pool = '/pool14/chandra/'

;copy data

spawn,'cp -fp ' + htdocs + 'acis/Fluence/all.dat     ' + alerts_dir + 'Data/.'
spawn,'cp -fp ' + htdocs + 'acis/Fluence/current.dat ' + alerts_dir + 'Data/.'
spawn,'cp -fp ' + htdocs + 'acis/Fluence/protons.dat ' + alerts_dir + 'Data/.'

spawn,'cp -fp ' + htdocs + 'acis/Fluence/all.dat     ' + web_dir + '.'
spawn,'cp -fp ' + htdocs + 'acis/Fluence/current.dat ' + web_dir + '.'
spawn,'cp -fp ' + htdocs + 'acis/Fluence/protons.dat ' + web_dir + '.'


readcol,crm3_dir     + "Data/CRMsummary.dat",infile,format="a",delimiter="ZZZ"
readcol,chandra_pool + "chandra_psi.snapshot",snapshot,format="a",delimiter="ZZZ"
readcol,ace_dir      + "Data/fluace.dat.good",ace,format="a",delimiter="ZZZ"
readcol,alerts_dir   + "Data/current.dat",acis,format="a",delimiter="ZZZ"
...
line= strsplit(infile(3),":",/extract)
goes_p4_flx = float(line(1))

line= strsplit(infile(4),":",/extract)
goes_p7_flx = float(line(1))

line= strsplit(infile(5),":",/extract)
goes_e2_flx = float(line(1))

line= strsplit(infile(6),":",/extract)
...
line= strsplit(infile(10),":",/extract)
...
iline = ifot_dir + '/radmon/*.rdb'
files = findfile(iline)
print, "Radmon file ", files(n_elements(files)-1)
rdb_dat=rdb_read(files(n_elements(files)-1))
;rdb_dat=rdb_read(files(n_elements(0)))
...
att_elapsed_time=max([calc_att(rad_start,jday, ifot_dir),0])
print, "CALCULATING RAD TIME"
att_time=max([calc_att(jday,rad_end,ifot_dir),0])
print, "CALCULATING COM TIME"
att_com_time=max([calc_att(jday,jcom_start(0),ifot_dir),0])
print, "CALCULATING COM2 TIME"
att_com2_time=max([calc_att(jday,jcom_start(1),ifot_dir),0])
...
goes_p4_flu= goes_p4_flx*elapsed_time
goes_p4_flx_att= goes_p4_flx*att_factor
goes_p4_flu_att= goes_p4_flu*att_flu_factor
goes_p4_pflu= goes_p4_flu+(goes_p4_flx*time_to_rad)
goes_p4_cflu= goes_p4_flu+(goes_p4_flx*time_to_com(0))
goes_p4_c2flu= goes_p4_flu+(goes_p4_flx*time_to_com(1))
goes_p4_pflu_att= goes_p4_flu_att+(goes_p4_flx*att_time)
goes_p4_cflu_att= goes_p4_flu_att+(goes_p4_flx*att_com_time)
goes_p4_c2flu_att= goes_p4_flu_att+(goes_p4_flx*att_com2_time)

goes_p7_flu= goes_p7_flx*elapsed_time
goes_p7_flx_att= goes_p7_flx*att_factor
goes_p7_flu_att= goes_p7_flu*att_flu_factor
goes_p7_pflu= goes_p7_flu+(goes_p7_flx*time_to_rad)
goes_p7_cflu= goes_p7_flu+(goes_p7_flx*time_to_com(0))
goes_p7_c2flu= goes_p7_flu+(goes_p7_flx*time_to_com(1))
goes_p7_pflu_att= goes_p7_flu_att+(goes_p7_flx*att_time)
goes_p7_cflu_att= goes_p7_flu_att+(goes_p7_flx*att_com_time)
goes_p7_c2flu_att= goes_p7_flu_att+(goes_p7_flx*att_com2_time)
...
;openw,  OUT, "/data/mta4/www/RADIATION_new/Alerts/rad_summ.html", /get_lun
openw,  OUT, "/data/mta4/www/RADIATION_new/Alerts/rad_summ_test.html", /get_lun
printf, OUT, '<!DOCTYPE html>'
printf, OUT, '<html>'
printf, OUT, '<head>'
printf, OUT, '<title>Chandra Radiation Summary</title>'
printf, OUT, '<meta http-equiv="refresh" content="300"/>'
printf, OUT, '</head>'
printf, OUT, '<body style="width:95%;margin-left:10px; margin-right;10px;background-color:#FAEBD7;font-family:Georgia, "Times New Roman", Times, serif">'

printf, OUT, '<div style="float:right;">'
printf, OUT, '<a href="../../sot.html">Back to the SOT home page</a>'
printf, OUT, '<br />'
printf, OUT, '<a href="../index.html">Back to Radiation Main Page</a>'
printf, OUT, '</div>'
...
printf, OUT, '<tr><th>&#160</th><th>Current Flux</th><th>Current fluence<br />(total so far in current orbit)</th><th>Projected fluence<br /> until next rad zone</th><th>Projected fluence</th><th>Projected fluence</th><th>Limits</th>'
...
printf, OUT, '<tr><td>GOES-R (P4)</td>'
printf, OUT, '<td>',string(goes_p4_flx_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_flu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_pflu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_cflu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_c2flu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>&#160</td></tr>'

printf, OUT, '<tr><td>GOES-R (P7)</td>'
printf, OUT, '<td>',string(goes_p7_flx_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_flu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_pflu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_cflu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_c2flu_att,format='(E9.3)'),'</td>'
printf, OUT, '<td>&#160</td></tr>'

printf, OUT, '<tr><td>GOES-R (E > 2.0 MeV)</td>'
...
printf, OUT, '<tr><th>&#160</th><th>Current Flux</th><th>Current fluence <br />(total so far in current orbit)</th><th>Projected fluence</th><th>Projected fluence</th><th>&#160</th><th>Limits</th>'
...
printf, OUT, '<tr><td>GOES-R (P4)</td>'
printf, OUT, '<td>',string(goes_p4_flx,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_flu,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_pflu,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_cflu,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p4_c2flu,format='(E9.3)'),'</td>'
printf, OUT, '<td>30.0/90.9 (flux, yellow/red)</td></tr>'

printf, OUT, '<tr><td>GOES-R (P7)</td>'
printf, OUT, '<td>',string(goes_p7_flx,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_flu,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_pflu,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_cflu,format='(E9.3)'),'</td>'
printf, OUT, '<td>',string(goes_p7_c2flu,format='(E9.3)'),'</td>'
...
printf, OUT, '<br /><br />'
printf, OUT, '<div style="text-align:center;">'
printf, OUT, '<img src="../Orbit/Plots/crmplatt.png" style="width:60%;">'
printf, OUT, '</div>'

printf, OUT, '<div style="padding-bottom:30px;"></div>'
