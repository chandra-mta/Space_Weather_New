...
*       last update: Spe 10, 2018
*
*       this version is modified for lunix, though it may run on others
*

      DOUBLE PRECISION EPOCH,DS50
      COMMON/E1/XMO,XNODEO,OMEGAO,EO,XINCL,XNO,XNDT2O,XNDD6O,BSTAR,
     1            X,Y,Z,XDOT,YDOT,ZDOT,JUNK,EPOCH,DS50
     ...
     5            -1.65597E-6,.743669161E-1,6378.135,1440.,1./
      DIMENSION ISET(5)
      dimension idtab(4,12)
      common/times/ilday(100)
     ...
     1     'Jul','Aug','Sep','Oct','Nov','Dec'/
      ...
      data (idtab(i, 1),i=1,4)/  0,  0,  0,  0/
      data (idtab(i, 2),i=1,4)/ 31, 31, 31, 31/
      data (idtab(i, 3),i=1,4)/ 60, 60, 60, 60/
      data (idtab(i, 4),i=1,4)/ 91, 90, 90, 90/
      data (idtab(i, 5),i=1,4)/121,120,120,120/
      data (idtab(i, 6),i=1,4)/152,151,151,151/
      data (idtab(i, 7),i=1,4)/182,181,181,181/
      data (idtab(i, 8),i=1,4)/213,212,212,212/
      data (idtab(i, 9),i=1,4)/244,243,243,243/
      data (idtab(i,10),i=1,4)/274,273,273,273/
      data (idtab(i,11),i=1,4)/305,304,304,304/
      data (idtab(i,12),i=1,4)/335,334,334,334/
...
*     SELECT EPHEMERIS TYPE AND OUTPUT TIMES
      ...
      S=AE*(1.+SO/XKMPER)
 2    WRITE(6,*) 'Enter Model(1-5) StartT(min) EndT dT, then TLE:'
      READ (5,*) IEPT,TS,TF,DELT
      WRITE (6,*) IEPT,TS,TF,DELT
      IF(IEPT.LE.0) STOP
      IDEEP=0
      istep = delt*60
      itstart = ts*60

c     setup an array of cumulative day counts for 100 years
c     to be used in the date routines s2ydhms and ydhms2s

      ilday(1) = 0
      do i = 2,100
         ilday(i) = ilday(i-1) + 365
         if (mod(i,4).eq.0) ilday(i) = ilday(i) + 1
      enddo

*     READ IN MEAN ELEMENTS FROM 2 CARD FORMAT

      READ (5,702) EPOCH,XNDT2O,XNDD6O,IEXP,BSTAR,IBEXP,XINCL,
     1        XNODEO,EO,OMEGAO,XMO,XNO
      iyear = epoch/1000
      idoy = epoch - iyear*1000
      iyear = iyear + 1900
      if (iyear.le.1950) iyear = iyear + 100
      ihr = (epoch - int(epoch))*24
      imin = mod(int((epoch - int(epoch))*1440),60)
      isec = mod((int(epoch - int(epoch))*86400),60)
      imon = 12
      i = mod(iyear,4)+1
      do j = 1,11
         if ((idoy.gt.idtab(i,j)).and.(idoy.le.idtab(i,j+1))) imon = j
      enddo
      imday = idoy - idtab(i,imon)
      call ydhms2s(itime,iyear,idoy,ihr,imin,isec)
      write (6,*) 'TLE EPOCH:',
     1   itime,' (UTS1970) = ',mon(imon),imday,iyear,idoy,ihr,imin,isec
   7  IF(XNO.LE.0.) STOP
      WRITE(6,704) ISET(IEPT)
...
*     PERIOD GE 225 MINUTES  IS DEEP SPACE
   ...
   60 X=X*XKMPER/AE
      ...
      call s2ydhms(itime,iyear,idoy,ihr,imin,isec)
      WRITE(6,705) itime,iyear,idoy,ihr,imin,isec,X,Y,Z,XDOT,YDOT,ZDOT
      TSINCE=TSINCE+DELT
      itime = itime + istep
  ...
  704 FORMAT(1X,A4,8H    Time,
     1     22X,6HX (km),7X,6HY (km),7X,6HZ (km),7X,
     1     9HVX (km/s),4X,9HVY (km/s),4X,9HVZ (km/s))
  705 FORMAT(I12,I5,I4,3I3,6F13.4)
  706 FORMAT(A80)
  707 FORMAT(79X,A1)
...
**************************************************************************************
**************************************************************************************
      subroutine s2ydhms(itime,iyr,idoy,ihr,imin,isec)
      
c     convert time in linear seconds to year,day_of_year,h,m,s
      ...
      iy = -999
      do i = 2,100
         if ((idays.ge.ilday(i-1)).and.(idays.lt.ilday(i))) iy = i
      enddo
      idoy = idays - ilday(iy-1)
      iyr = (iy-1) + 1970 - 1
...
**************************************************************************************
**************************************************************************************
      subroutine ydhms2s(itime,iyr,idoy,ihr,imin,isec)

c     convert time in year,day_of_year,h,m,s to linear seconds since 1970

      common/times/ilday(100)

*       there is one days descrepancy from the original complied
*       program; maybe due to the operation architecture difference.
*       here 1 day is subtracted to match the time (ti Feb 2015)
*
*       the above change was reversed Sep 10, 2018, ti
*
      itime = (ilday(iyr+1-1970)+idoy)*86400+ihr*3600+imin*60+isec
c      itime = (ilday(iyr+1-1970)+idoy-1)*86400+ihr*3600+imin*60+isec
...
*      DEEP SPACE                                         31 OCT 80
     ...
     1           XNDD6O,BSTAR,X,Y,Z,XDOT,YDOT,ZDOT,JUNK,EPOCH,DS50
      COMMON/C1/CK2,CK4,E6A,QOMS2T,S,TOTHRD,
     1           XJ3,XKE,XKMPER,XMNPDA,AE
      COMMON/C2/DE2RA,PI,PIO2,TWOPI,X3PIO2
      DOUBLE PRECISION EPOCH, DS50
...
*      S AND QOMS2T ARE ALTERED
   ...
   10 PINVSQ=1./(AODP*AODP*BETAO2*BETAO2)
     ...
     1         2.*D2+C1SQ))
*       it seems the orginal setting of iflag=0 from the second round
*       does not work; so keep the iflag as it is (ti Feb 2015)
*   90 IFLAG=0
   90 iflag = iflag
...
*      MOTION AND QUADRATIC VARIATION IN MEAN ANOMALY
   ...
   50 ISIMP=1
      EDOT=-TOTHRD*XNDTN*(1.-EO)
*       keep iflag to none zero so that the initialization still can
*       work even after second round   (ti  Feb 2015)
*   70 IFLAG=0
   70 IFLAG=iflag
