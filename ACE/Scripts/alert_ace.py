...
def alert_ace():
    ...
    sel = np.logical_and(ace_table['cxotime'].data >= two_hours_ago, ace_table[_P3_CHANNEL] > 0)
    data_select = ace_table[sel]
    p130f = np.mean(data_select[_P3_CHANNEL].data) * len(data_select) * 300 #: Calculates the fluence with available data.
    ...
    if p130f > ACE_P3_LIMIT:
        ...
        if (data_select[-1]['cxotime'].datetime - CxoTime(curr_viol['ace_p3']['cxotime']).datetime).days > 1:
            #
            #--- Last alert was more than one day ago. Therefore this is a new alerting instance
            #
            curr_viol['ace_p3'] = {'cxotime': int(data_select[-1]['cxotime'].secs), 'val': p130f}

            text_body = 'A Radiation violation of P3 (130KeV) has been observed by ACE\n'
            text_body += f'Observed = {p130f:.4e}\n'
            text_body += '(limit = fluence of 3.6e8 particles/cm2-ster-MeV within 2 hours)\n'
...
def _convert_time_format(year, month, day, hhmm):
    ...
    time = datetime.strptime(f"{year:04}:{month:02}:{day:02}:{hh:02}:{mm:02}", "%Y:%m:%d:%H:%M")
...
if __name__ == "__main__":
    ...
    if args.mode == 'test':
        ...
        if not os.path.isfile(f"{ACE_DATA_DIR}/ace_12h_archive"):
            os.system(f"cp /data/mta4/Space_Weather/ACE/Data/ace_12h_archive {ACE_DATA_DIR}")
        alert_ace()
