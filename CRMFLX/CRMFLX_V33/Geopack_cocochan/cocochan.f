...
c gfortran cocochan.f geopack.f supple.f -o cocochan
...
c      real x,y,z,vx,vy,vz,pi,re,Xgm,Ygm,Zgm,Xge,Yge,Zge
      ...
      open(unit=1,file='/data/mta4/proj/rac/ops/ephem/PE.EPH.dat')
      open(unit=2,file='/data/mta4/proj/rac/ops/ephem/PE.EPH.gsme')
      open(unit=3,file='/data/mta4/proj/rac/ops/ephem/PE.EPH.gsme_in_Re'
     &  )

      ios = 0
      do 100 while (ios.eq.0)
         read(1,*,iostat=ios) t,x,y,z,vx,vy,vz,fy,mon,d,h,mm,s,Xkp
...
c calculate cartesian coordinates using GEOPACK 
         ...
         call recalc_08(yr,doy,h,mm,s,vx, vy, vz)
...
C
C        call recalc_08(yr,doy,h,mm,s,vx, 0, 0)
...
c      if (ios.eq.0) write(2,3) t,R,Tgsm,Pgsm,Tgse,Pgse,fy,mon,d,h,mm,s
