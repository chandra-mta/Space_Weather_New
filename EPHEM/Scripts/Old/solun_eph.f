...
c     calculate solar and lunar geocentric ephemerides
c     for Chandra ephemeris times read from a file.
c     Uses the JPL DE405 ephemeris, and JPL code.

c     compile and link as follows:
c     f77 solun_eph.f -o solun_eph

c     Robert Cameron
c     April 2001

c
c     an updated ephemeris file lnxp1600p2200.405 was 
c     downloaded (t. isobe Feb 19, 2015)
c     see: http://ssd.jpl.nasa.gov/?planet_eph_export for details

      implicit none

      double precision t,x,y,z,vx,vy,vz,fyr,et,rs(6),rm(6)
      integer i,ios,mon,d,h,m,s

  
c     read the ephemeris times and write ECI(x,y,z) in Mm.

      open(unit=1,name=
     *'/data/mta4/Space_Weather/EPHEM/Data/PE.EPH.dat')

      open(unit=2,name=
     *'/data/mta4/Space_Weather/EPHEM/Data/solun_eph.dat') 

      do while (ios.eq.0)
         read(1,*,iostat=ios) t,x,y,z,vx,vy,vz,fyr,mon,d,h,m,s
         et = 2450814.5 + t/86400.0
         call pleph ( et, 11, 3, rs )
         call pleph ( et, 10, 3, rm )
         do i=1,3
            rs(i)=rs(i)*149597.870
            rm(i)=rm(i)*149597.870
         end do
         if (ios.eq.0) write(2,1) t,rs(1),rs(2),rs(3),rm(1),rm(2),rm(3)
      end do

 1    format(f13.1,3f15.6,3f12.6)
...
C     NAMFIL='/data/mta4/proj/rac/ops/ephem/unxp2000.405'
      NAMFIL=
     &'/data/mta4/Space_Weather/house_keeping/lnxp1600p2200.405'
...
CCC   IF(FIRST) CALL STATE(0.D0,0,0,0)
      FIRST=.FALSE.
