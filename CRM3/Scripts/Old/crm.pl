...
$crmnew_root = "$crm3_dir/Data/CRM3_p.dat";
...
open GF, $gp_dat or print STDERR "$0: Cannot open $gp_dat\n";
@gp = <GF>;
die "No GOES-p data in $gp_dat\n" unless (@gp);
@gp = split ' ',$gp[-1];
#$g15p2o = $gp[7];
#if ($#gp == 16) { $g15p2 = $gp[7]*0.011; $g15p5 = $gp[10]*1.41696 };
if ($#gp == 16) { $g15p2 = $gp[7]*3.3; $g15p5 = $gp[10]*12 };

# read the GOES-S data

open GF, $gs_dat or print STDERR "$0: Cannot open $gs_dat\n";
@gp = <GF>;
die "No GOES-s data in $gs_dat\n" unless (@gp);
@gp = split ' ',$gp[-1];
#$g13p2o = $gp[7];
#if ($#gp == 16) { $g13p2 = $gp[7]*0.011; $g13p5 = $gp[10]*1.41696 };
if ($#gp == 16) { $g13p2 = $gp[7]*3.3; $g13p5 = $gp[10]*12 };

open GF, $gpe_dat or print STDERR "$0: Cannot open $gpe_dat\n";
@gp = <GF>;
die "No GOES-p elec data in $gpe_dat\n" unless (@gp);
@gp = split ' ',$gp[-1];
$g13e2 = $gp[13];
...
#$txt .= sprintf "                GOES-P P2 flux (p/cm^2-s-sr-MeV) : %.2f\n",$g15p2o;
$txt .= sprintf "           GOES-P P2 flux, in RADMON P4GM  units : %.2f\n",$g15p2;
#$txt .= sprintf "                GOES-S P2 flux (p/cm^2-s-sr-MeV) : %.2f\n",$g13p2o;
$txt .= sprintf "           GOES-S P2 flux, in RADMON P4GM  units : %.2f\n",$g13p2;
$txt .= sprintf "           GOES-P P5 flux, in RADMON P41GM units : %.2f\n",$g15p5;
$txt .= sprintf "           GOES-S P5 flux, in RADMON P41GM units : %.2f\n",$g13p5;
$txt .= sprintf "           GOES-P E > 2.0 MeV flux (p/cm^2-s-sr) : %.1f\n",$g13e2;
...
$out = "$web_dir".'CRMsummary.dat';
system("cp $sumdat $out");
$out = "$web_dir".'CRMarchive.dat';
system("cp $arcdat $out");

# make the CRM radiation prediction plots

`/usr/local/bin/idl  $crm3_dir/Scripts/CRM_plots.idl > /dev/null 2>&1`;
